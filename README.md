...
All accepted commands (commands are case-insensitive; operands support direct `n`, immediate `#n`, or indirect `(n)` where applicable):
...
| `DIV n`   | Divides the ACC by the value at MC n (integer division) and stores the result into the ACC; raises an error on division by zero        |
| `DIV #n`  | Divides the ACC by the value n (integer division) and stores the result into the ACC; raises an error on division by zero              |
| `DIV (n)` | Divides the ACC by the value at the address stored in MC n (integer division) and stores the result into the ACC; raises on zero       |
